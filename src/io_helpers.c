...
void display_message(char *str) {
    write(STDOUT_FILENO, str, strnlen(str, MAX_STR_LEN * MAX_STR_LEN));
}
 ...
 */
ssize_t get_input(char *in_ptr, int pipeFlag) {
    int retval = 0;
    char* num = "";
    num = malloc(MAX_STR_LEN);
    if (pipeFlag == 0){
        retval = read(STDIN_FILENO, in_ptr, MAX_STR_LEN+1); // Not a sanitizer issue since in_ptr is allocated as MAX_STR_LEN+1
    }else{
        ssize_t sizer = (MAX_STR_LEN*MAX_STR_LEN)+1;
        retval = read(STDIN_FILENO, in_ptr, sizer*sizer); // Not a sanitizer issue since in_ptr is allocated as MAX_STR_LEN+1
    }
    int read_len = retval;
    ...
    if (read_len > MAX_STR_LEN && pipeFlag == 0) {
        read_len = 0;
        retval = -1;
...
size_t tokenize_input(char *in_ptr, char **tokens) {
    ...
    while (curr_ptr != NULL) {// TODO: Fix this
        tokens[token_count] = malloc(sizeof(char)*MAX_STR_LEN);
        strcpy(tokens[token_count],  curr_ptr);
